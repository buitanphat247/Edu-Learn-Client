    ...
    return (
      <main className="container mx-auto px-4 py-8">
        <div className="text-center py-12">
          ...
          <button onClick={() => router.push("/news")} className="bg-[#1c91e3] text-white px-6 py-2 rounded-lg hover:bg-[#0d7bc4] transition-colors">
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      <div className="mb-5 text-sm text-gray-600">
        ...
        <Link href="/news" className="hover:text-[#1c91e3] transition-colors">
          Tin tức và sự kiện
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          <article className="bg-white rounded-lg">
            <Tag color={news.category === "Tin tức" ? "blue" : "green"} className="mb-4">
              {news.category}
            </Tag>
              ...
              <div className="flex-1">
                <div className="prose prose-lg max-w-none">
                  {news.content.map((paragraph, index) => (
                    <p key={index} className={`text-gray-700 leading-relaxed mb-4 ${paragraph.startsWith("•") ? "ml-4" : ""}`}>
                      {paragraph}
                    </p>
                  ))}
                </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-gray-50 rounded-lg sticky top-24">
            <h2 className="text-xl font-bold text-gray-800 mb-6">Tin nổi bật</h2>

            <div className="mb-6 p-4 bg-linear-to-br from-blue-600 to-blue-800 rounded-lg text-white">
              <h3 className="text-2xl font-bold mb-2">BACK TO SCHOOL</h3>
              <div className="flex gap-2 mb-4 text-xs">
                <span className="bg-white/20 px-2 py-1 rounded">Ebook</span>
                <span className="bg-white/20 px-2 py-1 rounded">Podcast</span>
                <span className="bg-white/20 px-2 py-1 rounded">Flashcard</span>
                <span className="bg-white/20 px-2 py-1 rounded">Video</span>
                <span className="bg-white/20 px-2 py-1 rounded">Docs</span>
              </div>
              <button className="text-sm font-medium hover:underline">Khám Phá Ngay</button>
            </div>

            <div className="space-y-4 mb-6">
              {featuredNewsList.map((item, index) => (
                <div key={item.id} className="pb-4 border-b border-gray-300 last:border-0 last:pb-0">
                  {index === 0 && (
                    <Link href={`/news/${item.id}`} className="block mb-2">
                      <h3 className="text-base font-semibold text-gray-800 hover:text-[#1c91e3] transition-colors line-clamp-2 mb-2">{item.title}</h3>
                      <span className="text-sm text-[#1c91e3] hover:underline">Xem chi tiết &gt;</span>
                    </Link>
                  )}
                  {index > 0 && (
                    <Link href={`/news/${item.id}`} className="block">
                      <p className="text-xs text-gray-500 mb-1">{item.date}</p>
                      <h3 className="text-sm font-medium text-gray-800 hover:text-[#1c91e3] transition-colors line-clamp-2">{item.title}</h3>
                    </Link>
                  )}
                </div>
              ))}
            ...
            <div className="mb-6 p-4 bg-blue-600 rounded-lg text-white text-center">
              <div className="flex justify-center mb-2">
                <svg className="w-8 h-8 text-yellow-300" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                </svg>
              </div>
              <button className="block w-full bg-white text-blue-600 px-4 py-2 rounded-lg font-medium hover:bg-blue-50 transition-colors mb-2">
                Đăng ký ngay
              </button>
              <button className="block w-full border border-white text-white px-4 py-2 rounded-lg font-medium hover:bg-white/10 transition-colors">
                Thử ngay
              </button>
            </div>

            <div className="mb-6 p-4 bg-linear-to-b from-yellow-400 to-orange-400 rounded-lg text-white relative overflow-hidden">
              <h3 className="text-lg font-bold mb-2">Tuần lễ đọc sách</h3>
              <p className="text-sm mb-4">Khám phá thế giới sách</p>
              <div className="absolute bottom-0 right-0 w-20 h-20 bg-white/20 rounded-full -mr-10 -mb-10"></div>
            </div>

            <div className="space-y-3">
              <div className="p-3 bg-green-500 rounded-lg text-white text-sm flex items-center justify-between">
                <span>DLIB K12 Luôn bên bạn suốt mùa hè</span>
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z" />
                </svg>
              </div>
              <div className="p-3 bg-yellow-400 rounded-lg text-gray-800 text-sm text-center font-medium">Nội dung được cập nhật liên tục</div>
            </div>
          </div>
        </div>
      </div>
